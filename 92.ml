 ...
 * `jump n (x,y)` that returns all coordinates `(u,v)` to which a knight
 * can jump from `(x,y)` to on a `n`×`n` chessboard. And finally, represent
 * the solution of our problem as a list knight positions (the knight's
 * tour).
 *
 * (* example pending *);;
 *
